...
with app.app_context():
    class User( db.Model,UserMixin):
        ...
        try_time = db.Column(db.DateTime, default=datetime.now)
...
def dash():
    now = datetime.now()
    print(now)
    print(current_user.try_time)
    if current_user.tries>1 and (now - current_user.try_time > timedelta(minutes=1)):
...
def get_temp():
    if request.method =="POST":
        ...
        if response.status_code==200:
            ...
            current_user.try_time=datetime.now()
